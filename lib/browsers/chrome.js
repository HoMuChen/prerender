	...
	return new Promise((resolve, reject) => {
		...
		.then(() => {
			...
			var checkIfDone = () => {
				...
				Promise.all([
					this.checkIfDomIsReady(tab),
					this.checkIfPageIsDoneLoading(tab)
				]).then(([domIsReady, doneLoading]) => {
					if ((doneLoading && !finished)  || (domIsReady && !finished)) {
						finished = true;
...
chrome.checkIfDomIsReady = function(tab) {
	return new Promise((resolve, reject) => {
		if (!tab.prerender.waitForSelector || tab.prerender.waitForSelector === '') {
			return resolve(false);
		}

		tab.Runtime.evaluate({
			expression: `!!document.querySelector('${tab.prerender.waitForSelector}')`
		}).then((result) => {
			const ifDomExist = result && result.result && result.result.value;
